...
export default function RecentPosts() {
  ...
  return (
    <section aria-labelledby="recent-posts-title" className="container-max mt-16">
      <div className="flex items-center justify-between mb-6">
        <h2 id="recent-posts-title" className="section-title mb-0">
          Aktualności
        </h2>
        <Link
          to="/aktualnosci"
          className="text-sm text-brand dark:text-accent-orange hover:underline flex items-center gap-1"
        >
      ...
      {loading ? (
        ...
        <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {posts.map((post) => (
            <li key={post.id}>
              <article className="card overflow-hidden rounded-2xl shadow-sm hover:shadow-md transition h-full flex flex-col">
